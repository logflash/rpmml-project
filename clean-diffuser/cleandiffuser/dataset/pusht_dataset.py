...
from cleandiffuser.dataset.dataset_utils import (ImageNormalizer,
                                                 MinMaxNormalizer,
...
class PushTKeypointDataset(BaseDataset):
    ...
    def get_normalizer(self):
        agent_pos_normalizer = MinMaxNormalizer(
            self.replay_buffer["state"][:, :2]
        )  # (N, 2)
        keypoint_normalizer = MinMaxNormalizer(
            self.replay_buffer["keypoint"][:]
        )  # (N, 9, 2)
