...
class D4RLAntmazeDataset(BaseDataset):
    ...
    def __init__(
        ...
        noreaching_penalty: float = -100.0,
        discount: float = 0.99,
    ):
        super().__init__()
        ...
        for i in range(timeouts.shape[0]):
            ...
            if i != 0 and ((dones[i - 1] and not dones[i]) or timeouts[i - 1]):
                ...
                # 2. agent never reaches the goal during the episode
                    ...
                    raise ValueError(
                        f"path_length: {path_length} > max_path_length: {max_path_length}"
                    )
    ...
    def __init__(
        ...
        for i in range(timeouts.shape[0]):
            ...
            if i != 0 and ((dones[i - 1] and not dones[i]) or timeouts[i - 1]):
                ...
                max_starts = [
                    min(self.path_lengths[-1] - 1, max_path_length - horizon)
...
class DV_D4RLAntmazeSeqDataset(BaseDataset):
    ...
    def __init__(
        ...
        for index in timeouts_indices:
            ...
            if (
                terminal_index == -1
                and self.learn_policy
                and not only_learn_reached_policy
            ):
                path_length = index - ptr + 1
                assert path_length == max_path_length

                _seq_obs = np.zeros(
                    (max_path_length + (horizon - 1) * stride, self.o_dim),
                    dtype=np.float32,
                )
                _seq_act = np.zeros(
                    (max_path_length + (horizon - 1) * stride, self.a_dim),
                    dtype=np.float32,
                )
                _seq_rew = np.zeros(
                    (max_path_length + (horizon - 1) * stride, 1), dtype=np.float32
                )
                _seq_tml = np.zeros(
                    (max_path_length + (horizon - 1) * stride, 1), dtype=np.float32
                )
                ...
                _seq_obs[path_length:] = normed_observations[
                    terminal_index
                ]  # repeat state
