import pytest
import torch
from cleandiffuser.nn_diffusion import BaseNNDiffusion, ChiTransformer
...
def test_invalid_timestep_emb_type():
    with pytest.raises(AssertionError):
        ChiTransformer(
            ...
            p_drop_attn=0.3,
            n_cond_layers=2,
            timestep_emb_type="invalid_type",
        )
...
def test_chi_transformer_init_weights(model):
    for name, param in model.named_parameters():
        if "weight" in name:
            assert (
                param.data.std().item() < 0.1
            ), f"Weight init for {name} is not correct"
        if "bias" in name:
