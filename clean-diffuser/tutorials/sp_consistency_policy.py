...
if __name__ == "__main__":
    ...
    weight_temperature = 100.0  # 10 for me / 100 for m / 400 for mr
    mode = "iql_training"
    ...
    if mode == "iql_training":
        ...
        # Begin Training
        n_gradient_step = 0
        log = {"q_loss": 0.0, "v_loss": 0.0}
        for batch in loop_dataloader(dataloader):

            obs, next_obs = batch["obs"]["state"].to(device), batch["next_obs"][
                "state"
            ].to(device)
        ...
        nn_diffusion = IDQLMlp(
            obs_dim, act_dim, emb_dim=64, timestep_emb_type="untrainable_fourier"
        )
        ...
        n_gradient_step = 0
        log = {"bc_loss": 0.0, "unweighted_bc_loss": 0.0}
