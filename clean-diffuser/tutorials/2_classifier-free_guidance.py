...
from cleandiffuser.nn_condition import BaseNNCondition, MLPCondition, get_mask
...
class MyObsNNCondition(BaseNNCondition):
    ...
    def forward(self, condition: torch.Tensor, mask: torch.Tensor = None):
        mask = get_mask(
            mask, (condition.shape[0], 1), self.dropout, self.training, condition.device
        )
...
if __name__ == "__main__":
    ...
    nn_diffusion = PearceMlp(
        act_dim, To=1, emb_dim=64, hidden_dim=256, timestep_emb_type="positional"
    )
