...
if __name__ == "__main__":
    ...
    nn_diffusion = PearceMlp(
        act_dim, To=0, emb_dim=64, hidden_dim=256, timestep_emb_type="positional"
    )
    ...
    if use_customized_classifier:
        ...
        classifier = MSEClassifier(nn_classifier, temperature=1.0, device=device)
    ...
    avg_loss_diffusion, avg_loss_classifier = 0.0, 0.0
    for t in range(100000):
        ...
        if (t + 1) % 1000 == 0:
            print(
                f"[t={t + 1}] avg_loss_diffusion: {avg_loss_diffusion / 1000 :.3f} | "
