...
@hydra.main(
    config_path="../configs/idql/mujoco", config_name="mujoco", version_base=None
)
def pipeline(args):
    ...
    actor = DiscreteDiffusionSDE(
        ...
        x_max=+1.0 * torch.ones((1, act_dim)),
        x_min=-1.0 * torch.ones((1, act_dim)),
    ...
    # ------------------ Critic ---------------------
    iql_q = IDQLQNet(obs_dim, act_dim, hidden_dim=args.critic_hidden_dim).to(
        args.device
    )
    ...
    if args.mode == "train":
        ...
        n_gradient_step = 0
        log = {"bc_loss": 0.0, "q_loss": 0.0, "v_loss": 0.0}
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if n_gradient_step % 2 == 0:
                ...
                v = iql_v(obs)
                v_loss = (
                    torch.abs(args.iql_tau - ((q - v) < 0).float()) * (q - v) ** 2
                ).mean()
                ...
                for param, target_param in zip(
                    iql_q.parameters(), iql_q_target.parameters()
                ):
    ...
    # ---------------------- Inference ----------------------
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                ...
                obs = (
                    obs.unsqueeze(1).repeat(1, args.num_candidates, 1).view(-1, obs_dim)
                )
                ...
                print(
                    f"[t={t}] rew: {np.around((rew * (1 - cum_done)) if t < 1000 else rew, 2)}"
                )
