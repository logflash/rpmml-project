...
class SupportedActionD4RLMuJoCoTDDataset(D4RLMuJoCoTDDataset):
    ...
    def __init__(
        self,
        dataset: torch.Dict[str, ndarray],
        normalize_reward: bool = False,
        K: int = 16,
    ):
        super().__init__(dataset, normalize_reward)
        self.supported_act = torch.empty(
            (self.size, K, self.a_dim), dtype=torch.float32
        )
...
@hydra.main(
    config_path="../configs/qgpo/mujoco", config_name="mujoco", version_base=None
)
def pipeline(args):
    ...
    if mode == "bc_training":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if n_gradient_step % args.log_interval == 0:
                print(
                    f"Gradient step: {n_gradient_step}, BC Loss: {avg_bc_loss / args.log_interval:.6f}"
                )
                avg_bc_loss = 0.0
        ...
        nn_classifier = QGPONNClassifier(
            obs_dim, act_dim, 64, [256, 256, 256], "untrainable_fourier"
        )
        clf = QGPOClassifier(
            nn_classifier,
            ema_rate=args.ema_rate,
        ...
        q.load_state_dict(
            torch.load(save_path + "q_ckpt_latest.pt", map_location=device)
        )
        ...
        sampling_steps = (
            10 if env_name == "hopper-medium-replay-v2" else args.sampling_steps
        )
        ...
        for i in range(num_episodes):
            ...
            while not np.all(all_done) and t < 1000 + 1:
                ...
                obs = torch.tensor(normalizer.normalize(obs), dtype=torch.float32).to(
                    device
                )
                ...
                logp = einops.rearrange(
                    log["log_p"], "(b k) 1 -> b k", k=num_candidates
                )
                ...
                print(
                    f"[t={t}] rew: {np.around((rew * (1 - all_done)) if t < 1000 else rew, 2)}"
                )
