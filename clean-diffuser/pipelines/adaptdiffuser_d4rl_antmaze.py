...
@hydra.main(
    config_path="../configs/adaptdiffuser/antmaze",
    config_name="antmaze",
    version_base=None,
)
def pipeline(args):
    ...
    if args.mode == "train":
        ...
        diffusion_lr_scheduler = CosineAnnealingLR(
            agent.optimizer, args.diffusion_gradient_steps
        )
        classifier_lr_scheduler = CosineAnnealingLR(
            agent.classifier.optim, args.classifier_gradient_steps
        )
    ...
    # ---------------------- Finetune ------------------------
        ...
        traj_buffer = torch.empty(
            (50000, args.task.horizon, obs_dim + act_dim), device=args.device
        )
        ...
        for batch in loop_dataloader(gen_dl):
            ...
            prior = torch.zeros(
                (sample_bs, args.task.horizon, obs_dim + act_dim), device=args.device
            )
            ...
            print(
                f"{num_selected} of 10000 trajs have been selected. Progress: {ptr} / {50000}"
            )
            if ptr == 50000:
                break
        ...
        while n_gradient_step < 200_000:
            ...
            if (n_gradient_step + 1) % 5_000 == 0:
                agent.save(
                    save_path + f"finetuned_diffusion_ckpt_{n_gradient_step + 1}.pt"
                )
    ...
    # ---------------------- Inference ----------------------
        ...
        prior = torch.zeros(
            (args.num_envs, args.task.horizon, obs_dim + act_dim), device=args.device
        )
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                ...
                idx = logp.argmax(0)
                act = traj.view(
                    args.num_candidates, args.num_envs, args.task.horizon, -1
