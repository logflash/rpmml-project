...
def make_env(args, idx):
    def thunk():
        ...
        env = VideoRecordingWrapper(
            env, video_recorder, file_path=None, steps_per_render=1
        )
    ...
    for i in range(args.eval_episodes // args.num_envs):
        ...
        while t < args.max_episode_steps:
            if args.env_name == "pusht-v0":
                ...
                obs_seq = obs[:, : args.obs_steps, :].astype(
                    np.float32
                )  # (num_envs, obs_steps, obs_dim)
                ...
                nkeypoint = dataset.normalizer["obs"]["keypoint"].normalize(
                    keypoint
                )  # (num_envs*obs_staps*9, 2)
                nkeypoint = nkeypoint.reshape(
                    args.num_envs, args.obs_steps, keypoint_pos_dim
                )  # (num_envs, obs_staps, 18)
                # agent_pos
                nagent_pos = dataset.normalizer["obs"]["agent_pos"].normalize(
                    agentpos_obs_seq
                )  # (num_envs, obs_steps, 2)
                nobs = np.concatenate(
                    (nkeypoint, nagent_pos), axis=-1
                )  # (num_envs, obs_steps, obs_dim)
                nobs = torch.tensor(
                    nobs, device=args.device, dtype=torch.float32
                )  # (num_envs, obs_steps, obs_dim)
            with torch.no_grad():
                if args.nn == "pearce_mlp" or args.nn == "pearce_transformer":
                    ...
                    prior = torch.zeros(
                        (args.num_envs, args.action_dim), device=args.device
                    )
                elif args.nn == "dit":
                    ...
                    prior = torch.zeros(
                        (args.num_envs, args.action_steps, args.action_dim),
                        device=args.device,
                    )
                else:
                    ValueError("fatal nn")
            ...
            naction = (
                naction.detach().to("cpu").clip(-1.0, 1.0).numpy()
            )  # (num_envs, action_dim)
            action_pred = dataset.normalizer["action"].unnormalize(naction)
            action = action_pred.reshape(
                args.num_envs, 1, args.action_dim
        ...
        print(
            f"[Episode {1+i*(args.num_envs)}-{(i+1)*(args.num_envs)}] reward: {ep_reward} success:{success}"
        )
...
def pipeline(args):
    ...
    if args.nn == "pearce_mlp":
        ...
        nn_diffusion = PearceMlp(
            act_dim=args.action_dim, To=args.obs_steps, emb_dim=128, hidden_dim=512
        ).to(args.device)
        nn_condition = PearceObsCondition(obs_dim=args.obs_dim, dropout=0.0).to(
            args.device
        )
    elif args.nn == "dit":
