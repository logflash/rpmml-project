...
@hydra.main(
    config_path="../configs/dd/antmaze", config_name="antmaze", version_base=None
)
def pipeline(args):
    ...
    # ---------------------- Inference ----------------------
        ...
        prior = torch.zeros(
            (args.num_envs, args.task.horizon, obs_dim), device=args.device
        )
        condition = (
            torch.ones((args.num_envs, 1), device=args.device) * args.task.target_return
        )
