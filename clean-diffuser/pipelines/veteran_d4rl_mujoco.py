...
@hydra.main(
    config_path="../configs/veteran/mujoco", config_name="mujoco", version_base=None
)
def pipeline(args):
    ...
    # base config
    base_path = (
        f"{args.pipeline_name}_H{args.task.planner_horizon}_Jump{args.task.stride}"
    )
    ...
    if args.guidance_type == "MCSS":
        ...
        if args.planner_net == "transformer":
            nn_condition_planner = MLPCondition(
                in_dim=1,
                out_dim=args.planner_emb_dim,
                hidden_dims=[
                    args.planner_emb_dim,
                ],
                act=nn.SiLU(),
                dropout=0.25,
            )
    ...
    if args.pipeline_type == "separate":
        if args.use_diffusion_invdyn:
            ...
            invdyn = MlpInvDynamic(
                obs_dim, act_dim, 512, nn.Tanh(), {"lr": 2e-4}, device=args.device
            )

    # ---------------------- Training ----------------------
    if args.mode == "train":
        # Planner
        planner_lr_scheduler = CosineAnnealingLR(
            planner.optimizer, args.planner_diffusion_gradient_steps
        )
        planner.train()
        ...
        if args.guidance_type == "MCSS":
            critic_lr_scheduler = CosineAnnealingLR(
                critic_optim, args.planner_diffusion_gradient_steps
            )
            critic.train()
        elif args.guidance_type == "cg":
            classifier_lr_scheduler = CosineAnnealingLR(
                planner.classifier.optim, args.planner_diffusion_gradient_steps
            )
            classifier.train()
        ...
        if args.pipeline_type == "separate":
            if args.use_diffusion_invdyn:
                policy_lr_scheduler = CosineAnnealingLR(
                    policy.optimizer, args.policy_diffusion_gradient_steps
                )
                policy.train()
            else:
                invdyn_lr_scheduler = CosineAnnealingLR(
                    invdyn.optim, args.invdyn_gradient_steps
                )
                invdyn.train()
        ...
        pbar = tqdm(
            total=max(
                args.planner_diffusion_gradient_steps,
                args.policy_diffusion_gradient_steps,
            )
            / args.log_interval
        )
        for planner_batch, policy_batch in loop_two_dataloaders(
            planner_dataloader, policy_dataloader
        ):
            ...
            planner_horizon_obs_action = torch.cat(
                [planner_horizon_obs, planner_horizon_action], -1
            )
            planner_horizon_data = (
                planner_horizon_obs
                if args.pipeline_type == "separate"
                else planner_horizon_obs_action
            )
            ...
            policy_td_obs, policy_td_next_obs, policy_td_act = (
                policy_horizon_obs[:, 0, :],
                policy_horizon_obs[:, 1, :],
                policy_horizon_action[:, 0, :],
            )

            # ----------- Planner Gradient Step ------------
            if n_gradient_step <= args.planner_diffusion_gradient_steps:
                if args.guidance_type == "cfg":
                    log["avg_loss_planner"] += planner.update(
                        planner_horizon_data, planner_td_val
                    )["loss"]
                else:
                    if args.use_weighted_regression:
                        weighted_regression_tensor = torch.exp(
                            (planner_td_val - 1) * args.weight_factor
                        )
                        log["avg_loss_planner"] += planner.update(
                            planner_horizon_data,
            ...
            if args.guidance_type == "MCSS":
                ...
                if n_gradient_step <= args.planner_diffusion_gradient_steps:
                    log["avg_loss_classifier"] += planner.update_classifier(
                        planner_horizon_data, planner_td_val
                    )["loss"]
            ...
            if args.pipeline_type == "separate":
                if args.use_diffusion_invdyn:
                    ...
                    if n_gradient_step <= args.policy_diffusion_gradient_steps:
                        log["bc_loss_policy"] += policy.update(
                            policy_td_act,
                    ...
                    if n_gradient_step <= args.invdyn_gradient_steps:
                        log["bc_loss_policy"] += invdyn.update(
                            policy_td_obs, policy_td_act, policy_td_next_obs
                        )["loss"]
            ...
            if (n_gradient_step + 1) % args.save_interval == 0:
                ...
                if args.guidance_type == "MCSS":
                    ...
                    planner.classifier.save(
                        save_path + f"classifier_ckpt_{n_gradient_step + 1}.pt"
                    )
            ...
            n_gradient_step += 1
            if (
                n_gradient_step >= args.planner_diffusion_gradient_steps
                and n_gradient_step >= args.policy_diffusion_gradient_steps
            ):
                break
        ...
        n_gradient_step = 0
        log = dict.fromkeys(["loss_v", "v_mean"], 0.0)
        pbar = tqdm(total=MAX_STEPS / args.log_interval)
        for batch in loop_dataloader(td_dataloader):
            obs, next_obs = batch["obs"]["state"].to(args.device), batch["next_obs"][
                "state"
            ].to(args.device)
            ...
            if (n_gradient_step + 1) % args.save_interval == 0:
                torch.save(
                    {"ev": EV.state_dict()},
                    save_path + f"EV_ckpt_{n_gradient_step + 1}.pt",
                )
    ...
    # ---------------------- Inference ----------------------
        ...
        if args.guidance_type == "MCSS":
            ...
            planner.load(save_path + f"planner_ckpt_{args.planner_ckpt}.pt")
            # load classifier
            planner.classifier.load(
                save_path + f"classifier_ckpt_{args.planner_ckpt}.pt"
            )
            planner.eval()
            # load policy
            if args.pipeline_type == "separate":
            ...
            while not np.all(cum_done) and t < args.task.max_path_length + 1:
                ...
                if args.guidance_type == "MCSS":
                    planner_prior = torch.zeros(
                        (
                            args.num_envs * args.planner_num_candidates,
                            args.task.planner_horizon,
                            planner_dim,
                        ),
                        device=args.device,
                    ...
                    obs_repeat = (
                        obs.unsqueeze(1)
                        .repeat(1, args.planner_num_candidates, 1)
                        .view(-1, obs_dim)
                    )
                    ...
                    planner_prior = torch.zeros(
                        (args.num_envs, args.task.planner_horizon, planner_dim),
                        device=args.device,
                    )
                    condition = (
                        torch.ones((args.num_envs, 1), device=args.device)
                        * args.task.planner_target_return
                    )
                ...
                if args.pipeline_type == "separate":
                    if args.use_diffusion_invdyn:
                        policy_prior = torch.zeros(
                            (args.num_envs, act_dim), device=args.device
                        )
                        with torch.no_grad():
                            ...
                            act, log = policy.sample(
                                ...
                                condition_cfg=torch.cat(
                                    [obs_policy, next_obs_policy], dim=-1
                                ),
                                w_cfg=1.0,
                ...
                ep_reward += (
                    (rew * (1 - cum_done)) if t < args.task.max_path_length else rew
                )
