...
from cleandiffuser.dataset.d4rl_kitchen_dataset import (
    D4RLKitchenTDDataset, MultiHorizonD4RLKitchenDataset)
...
from cleandiffuser.utils import dict_apply, loop_dataloader, set_seed
...
@hydra.main(
    config_path="../configs/diffuserlite/kitchen",
    config_name="kitchen",
    version_base=None,
)
def pipeline(args):
    ...
    tgts = (
        [0.3, 0.35, 0.4, 0.5]
        if "mixed" in args.task.env_name
        else [0.25, 0.35, 0.45, 0.5]
    )
    ...
    invdyn = FancyMlpInvDynamic(
        obs_dim, act_dim, 256, nn.Tanh(), add_dropout=True, device=args.device
    )
    ...
    if args.mode == "iql_training":
        ...
        iql = IQL(obs_dim, act_dim, hidden_dim=512, discount=args.discount, tau=0.7).to(
            args.device
        )
        iql.train()
        ...
        for batch in loop_dataloader(td_dataloader):
            ...
            rew = batch["rew"].to(args.device) / 4.0
        ...
        lr_schedulers = [
            torch.optim.lr_scheduler.CosineAnnealingLR(
                diffusions[i].optimizer, args.diffusion_gradient_steps
            )
            for i in range(n_levels)
        ]
        ...
        n_gradient_step = 0
        log = dict.fromkeys(
            [f"loss{i}" for i in range(n_levels)] + ["invdyn_loss"], 0.0
        )
        disc_tensor = (
            args.discount
            ** torch.arange(temporal_horizons[0], device=args.device)[
                None, :, None
            ].float()
        )
        for batch in loop_dataloader(dataloader):
            for i in range(n_levels):
                ...
                obs = batch_data["obs"]["state"][
                    :, :: (temporal_horizons[i + 1] - 1) if i < n_levels - 1 else 1
                ].to(args.device)
                act = batch_data["act"][
                    :, :: (temporal_horizons[i + 1] - 1) if i < n_levels - 1 else 1
                ].to(args.device)
                rew = batch[i]["data"]["rew"].to(args.device) / 4.0
                pred_val = batch[i]["data"]["pred_val"].to(args.device)

                if i == 0:
                    rew[:, -1] = pred_val[:, -1]
                    val = (disc_tensor * rew).sum(1) / 100.0
                else:
                    val = rew.mean(1)
                ...
                if i == n_levels - 1 and n_gradient_step < args.invdyn_gradient_steps:
                    log[f"invdyn_loss"] += invdyn.update(
                        obs[:, :-1], act[:, :-1], obs[:, 1:]
                    )["loss"]
            ...
            if (n_gradient_step + 1) % args.save_interval == 0:
                for i in range(n_levels):
                    diffusions[i].save(
                        save_path + f"diffusion{i}_ckpt_{n_gradient_step + 1}.pt"
                    )
        ...
        for i in range(n_levels):
            diffusions[i].load(
                save_path + f"diffusion{i}_ckpt_{args.reflow_backbone_ckpt}.pt"
            )
            diffusions[i].eval()
            traj_cond_dataset.append(
                torch.zeros(
                    (args.cond_dataset_size, 2, planning_horizons[i], obs_dim),
                    device=args.device,
                )
            )
            ...
            traj_uncond_dataset.append(
                torch.zeros(
                    (args.uncond_dataset_size, 2, planning_horizons[i], obs_dim),
                    device=args.device,
                )
            )
            priors.append(
                torch.zeros(
                    (args.dataset_prepare_batch_size, planning_horizons[i], obs_dim),
                    device=args.device,
                )
            )

        dataloader = DataLoader(
            dataset, batch_size=args.dataset_prepare_batch_size, drop_last=True
        )
        ...
        for batch in loop_dataloader(dataloader):
            for i in range(n_levels):
                ...
                if i == 0:
                    ...
                    val = (
                        args.discount
                        ** torch.arange(rew.shape[1], device=args.device)[
                            None, :, None
                        ].float()
                        * rew
                    ).sum(1) / 100.0
        ...
        with h5py.File(save_path + "traj_cond_dataset.h5", "w") as f:
            for i in range(n_levels):
                f.create_dataset(
                    f"traj_cond_dataset_{i}", data=traj_cond_dataset[i].cpu().numpy()
                )
                f.create_dataset(
                    f"condition_dataset_{i}", data=condition_dataset[i].cpu().numpy()
                )
        with h5py.File(save_path + "traj_uncond_dataset.h5", "w") as f:
            for i in range(n_levels):
                f.create_dataset(
                    f"traj_uncond_dataset_{i}",
                    data=traj_uncond_dataset[i].cpu().numpy(),
                )
        ...
        with h5py.File(save_path + "traj_cond_dataset.h5", "r") as f:
            for i in range(n_levels):
                traj_cond_dataset.append(
                    torch.tensor(f[f"traj_cond_dataset_{i}"][:], device=args.device)
                )
                condition_dataset.append(
                    torch.tensor(f[f"condition_dataset_{i}"][:], device=args.device)
                )
        with h5py.File(save_path + "traj_uncond_dataset.h5", "r") as f:
            for i in range(n_levels):
                traj_uncond_dataset.append(
                    torch.tensor(f[f"traj_uncond_dataset_{i}"][:], device=args.device)
                )
        ...
        for n_gradient_step in range(args.reflow_gradient_steps):
            for i in range(n_levels):
                if (n_gradient_step % 5) == 0:
                    idx = torch.randint(
                        args.uncond_dataset_size, (args.batch_size,), device=args.device
                    )
                    ...
                    idx = torch.randint(
                        args.cond_dataset_size, (args.batch_size,), device=args.device
                    )
            ...
            if (n_gradient_step + 1) % args.save_interval == 0:
                for i in range(n_levels):
                    diffusions[i].save(
                        save_path + f"reflow{i}_ckpt_{n_gradient_step + 1}.pt"
                    )
        ...
        priors = [
            torch.zeros(
                (args.num_envs, planning_horizons[i], obs_dim), device=args.device
            )
            for i in range(n_levels)
        ]
        priors[0] = torch.zeros(
            (args.num_envs, n_candidates, planning_horizons[0], obs_dim)
        ).to(args.device)
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 280 + 1:
                ...
                target_return = torch.tensor(target_return, device=args.device)[
                    :, None, None
                ]
                ...
                for j in range(n_levels):
                    traj, _ = diffusions[j].sample(
                        ...
                        n_samples=(
                            args.num_envs * n_candidates if j == 0 else args.num_envs
                        ),
                        sample_steps=2 if args.test_model == "R2" else 5,
                        use_ema=args.use_ema,
                        condition_cfg=(
                            this_condition.reshape(-1, 1)
                            if j == 0
                            else this_condition[:, 0]
                        ),
                        w_cfg=w_cfgs[j],
