...
from cleandiffuser.utils import (GaussianNormalizer, Mlp, dict_apply,
                                 loop_dataloader, set_seed)
...
@hydra.main(
    config_path="../configs/sfbc/mujoco", config_name="mujoco", version_base=None
)
def pipeline(args):
    set_seed(args.seed)
    ...
    critic = Mlp(
        obs_dim + act_dim, [args.hidden_dim, args.hidden_dim], 1, torch.nn.SiLU()
    ).to(device)
    ...
    if args.mode == "bc_training":
        ...
        for k in range(args.q_training_iters):
            ...
            if k > 0:
                ...
                for i in tqdm(range(dataset.normed_seq_val.shape[0])):
                    obs = (
                        torch.tensor(dataset.seq_obs[i], device=device)
                        .unsqueeze(1)
                        .repeat(1, M, 1)
                    )
                ...
                target_seq_val[:, :-1] = dataset.seq_rew[
                    :, :-1
                ] + args.discount * np.maximum(
                ...
                dataset.normed_seq_val = dataset.val_normalizer.normalize(
                    target_seq_val
                )
                dataloader = DataLoader(
                    dataset,
            ...
            for batch in loop_dataloader(dataloader):
                ...
                critic_loss = (
                    (critic(torch.cat([obs, act], dim=-1)) - val).pow(2).mean()
                )
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                ...
                act, _ = actor.sample(
                    prior.view(-1, act_dim),
                    solver=args.solver,
                    n_samples=num_candidates * args.num_envs,
                    sample_steps=args.sampling_steps,
                    use_ema=True,
                    condition_cfg=condition.view(-1, obs_dim),
                    w_cfg=1.0,
                ...
                with torch.no_grad():
                    ...
                    act = act.gather(
                        1, sorted_indices.unsqueeze(-1).expand(-1, -1, act.size(-1))
                    )
