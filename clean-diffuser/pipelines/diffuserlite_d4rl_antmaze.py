...
@hydra.main(
    config_path="../configs/diffuserlite/antmaze",
    config_name="antmaze",
    version_base=None,
)
def pipeline(args):
        ...
        for batch in loop_dataloader(dataloader):
            for i in range(n_levels):
                ...
                if i == 0:
                    val = rew - 1.0
                    val[:, -1] = pred_val[:, -1]
                    val = (disc_tensor * val * mask).sum(1) / 100.0 + 1
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < 1000 + 1:
                ...
                if "medium-play" in args.task.env_name:
                    target_return[:] = 0.2
                    target_return[obs[:, 1] > 18.0] = 0.8
                elif "medium-diverse" in args.task.env_name:
                    target_return[:] = 0.2
                    target_return[obs[:, 0] > 10.0] = 0.3
                    target_return[obs[:, 1] > 15.0] = 0.8
                elif "large-play" in args.task.env_name:
                    target_return[:] = 0.6
                    target_return[np.logical_and(obs[:, 0] >= 13.0, obs[:, 1] < 28)] = (
                        0.25
                    )
                    target_return[obs[:, 0] < 13.0] = 0.1
                elif "large_diverse" in args.task.env_name:
                    target_return[:] = 0.6
                    target_return[np.logical_and(obs[:, 0] >= 13.0, obs[:, 1] < 28)] = (
                        0.3
                    )
                    target_return[obs[:, 0] < 13.0] = 0.25
