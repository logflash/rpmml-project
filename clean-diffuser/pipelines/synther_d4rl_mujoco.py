...
from cleandiffuser.utils import FreezeModules, report_parameters
...
class SynthERD4RLMuJoCoTDDataset(D4RLMuJoCoTDDataset):
    def __init__(self, save_path, dataset, normalize_reward: bool = False):
        ...
        extra_actions = extra_transitions[:, self.o_dim : self.o_dim + self.a_dim].clip(
            -1.0, 1.0
        )
        extra_rewards = extra_transitions[:, self.o_dim + self.a_dim]
        extra_next_observations = extra_transitions[
            :, self.o_dim + self.a_dim + 1 : self.o_dim * 2 + self.a_dim + 1
        ]
        ...
        normed_next_observations = np.concatenate(
            [
                self.normalizers["state"].normalize(next_observations),
                extra_next_observations,
            ],
            0,
        )
...
class TD3BC:
    def __init__(
        ...
        self.critic_target = (
            deepcopy(self.critic).requires_grad_(False).eval().to(device)
        )
    ...
    def update(self, obs, act, rew, next_obs, tml, update_actor: bool = False):
        ...
        with torch.no_grad():
            ...
            target_q = rew + (1.0 - tml) * 0.99 * self.critic_target(next_obs, next_act)
...
@hydra.main(
    config_path="../configs/synther/mujoco", config_name="mujoco", version_base=None
)
def pipeline(args):
    set_seed(args.seed)
    ...
    if args.mode == "train_diffusion":
        ...
        lr_scheduler = CosineAnnealingLR(
            synther.optimizer, T_max=args.diffusion_gradient_steps
        )

        synther.train()

        n_gradient_step = 0
        log = {"avg_diffusion_loss": 0.0}
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if (n_gradient_step + 1) % args.log_interval == 0:
                ...
                print(log)
                log = {"avg_diffusion_loss": 0.0, "gradient_steps": 0}
    ...
    # --------------------- Train RL ------------------------
        ...
        dataset = SynthERD4RLMuJoCoTDDataset(
            save_path, d4rl.qlearning_dataset(env), args.normalize_reward
        )
        dataloader = DataLoader(
            dataset,
            batch_size=args.batch_size,
        ...
        for batch in loop_dataloader(dataloader):
            ...
            _log = td3bc.update(
                obs, act, rew, next_obs, tml, bool(n_gradient_step % td3bc.policy_freq)
            )
