...
@hydra.main(
    config_path="../configs/edp/antmaze", config_name="antmaze", version_base=None
)
def pipeline(args):
    ...
    if args.mode == "train":
        ...
        n_gradient_step = 0
        log = {"bc_loss": 0.0, "q_loss": 0.0, "critic_loss": 0.0, "target_q_mean": 0.0}
        ...
        for batch in loop_dataloader(dataloader):
            ...
            next_act, _ = actor.sample(
                prior.repeat(10, 1),
                solver=args.solver,
                n_samples=args.batch_size * 10,
                sample_steps=args.sampling_steps,
                use_ema=True,
                temperature=1.0,
                condition_cfg=next_obs_rpt,
                w_cfg=1.0,
                requires_grad=False,
            )
            ...
            t = torch.randint(
                args.diffusion_steps, (args.batch_size,), device=args.device
            )
            eps = torch.randn_like(act)

            alpha, sigma = at_least_ndim(actor.alpha[t], act.dim()), at_least_ndim(
                actor.sigma[t], act.dim()
            )
