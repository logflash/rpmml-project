...
from cleandiffuser.env.robomimic.robomimic_image_wrapper import \
...
def make_async_envs(args):
    ...
    def dummy_env_fn():
        env = create_robomimic_env(
            env_meta=env_meta, shape_meta=args.shape_meta, enable_render=False
        )
        return MultiStepWrapper(
            VideoRecordingWrapper(
                RobomimicImageWrapper(
...
def inference(args, envs, dataset, agent, logger):
    ...
    for i in range(args.eval_episodes // args.num_envs):
        ...
        print(
            f"[Episode {1+i*(args.num_envs)}-{(i+1)*(args.num_envs)}] reward: {np.around(ep_reward, 2)} success:{success}"
        )
...
@hydra.main(
    config_path="../configs/dbc/robomimic_multi_modal/pearce_mlp", config_name="lift"
)
def pipeline(args):
    # ---------------- Create Logger ----------------
    set_seed(args.seed)
    ...
    if args.nn == "pearce_mlp":
        ...
        nn_diffusion = PearceMlp(
            act_dim=args.action_dim, To=args.obs_steps, emb_dim=256, hidden_dim=512
        ).to(args.device)
    ...
    print(
        f"======================= Parameter Report of Condition Model ======================="
    )
