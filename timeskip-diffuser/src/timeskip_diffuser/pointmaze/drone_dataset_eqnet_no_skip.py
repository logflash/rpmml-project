...
from an .npz archive containing position-only trajectories.
...
Attributes:
    ...
    def __init__(self, file_path, horizon=32):
        ...
        assert data.shape[2] == 3, f"expected traj_dim=3, got {data.shape[2]}"
        ...
        self.action_dim = 0         # no actions
        self.traj_dim = 3           # position-only
        ...
        # these no longer exist in plain dataset
        # Full normalization (same as position normalization)
        self.mean = self.pos_mean
        self.std  = self.pos_std
    ...
    def train(self, epochs=1000, save_every=100):
        ...
        for epoch in range(epochs):
            ...
            if save_every > 0 and (epoch + 1) % save_every == 0:
                checkpoint_path = f"checkpoints/no_skip_{epoch+1}_drone.pt"
...
class DiffuserPlanner:
    ...
    def plan_and_reconstruct(
        ...
        """
        Position-only planner.

        1. Perform diffusion planning (coarse trajectory of H positions)
        2. Denormalize positions
        3. Return coarse trajectory
        """
        ...
        start_norm_pos = torch.tensor(start_norm_pos, device=self.device)
        ...
        if condition_on_start:
            ...
            condition_value[0, 0, :] = start_norm_pos
        ...
        if goal_obs is not None and condition_on_goal:
            ...
            goal_norm_pos = torch.tensor(goal_norm_pos, device=self.device)

            condition_mask[0, -1, :] = True
            condition_value[0, -1, :] = goal_norm_pos
        ...
        coarse_norm = coarse_norm.cpu().numpy()[0]  # (H, state_dim)

        # ------------------------------------------------------------------
        # 2. DENORMALIZE POSITIONS
        # ------------------------------------------------------------------
        pos = coarse_norm * self.dataset.flat_std + self.dataset.flat_mean
        ...
        return dict(
            coarse=pos,        # (H, 3)
            coarse_pos=pos,    # alias for compatibility
        )
